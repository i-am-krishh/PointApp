...
PropertyPanel::PropertyPanel(QWidget* parent) : QWidget(parent) {
    ...
    // Connect shape list selection to load shape properties
    connect(shapeList, &QListWidget::itemClicked,
        this, &PropertyPanel::onShapeListItemSelected);

}

void PropertyPanel::onShapeAdded(BaseShape* shape)
{
    if (!shape) return;
...
void PropertyPanel::onShapeListItemSelected(QListWidgetItem* item)
{
    if (!item) return;
